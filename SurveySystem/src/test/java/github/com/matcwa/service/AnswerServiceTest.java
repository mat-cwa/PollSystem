...
import github.com.matcwa.model.entity.*;
...
class AnswerServiceTest {
    ...
    @Test
    void shouldCreateNewAnswerAndReturnNullErrorWhenUserIsPollsOwner() {
        //given
    ...
    @Test
    void shouldCreateNewAnswerAndReturnNullErrorWhenUserHasRoleAdmin() {
        //given
        NewAnswerDto newAnswer = new NewAnswerDto("newAnswer");
        User user = new User("user", "password");
        User owner = new User();
    ...
    void shouldReturnDeleteSuccessResponseDtoAndNullAnswerErrorDtoWhenUserIsAnswersOwner() {
        ...
        given(userRepository.findByUsername(tokenService.getUsernameFromToken("token"))).willReturn(Optional.of(user));
        //when
        ErrorHandling<SuccessResponseDto, AnswerError> response = answerService.deleteAnswer(1L, "token");
        //then
